...
public interface ClassroomRepository extends JpaRepository<Classroom, Long> {
    ...
    @Query("SELECT r FROM Reservation r WHERE MONTH(r.startDate) = :month AND r.classroom.id = :classroomId AND r.user.role = 'TEACHER'")
    ...
    @Query("SELECT r FROM Reservation r WHERE MONTH(r.startDate) = :month AND DAY(r.startDate) = :day AND r.classroom.id = :classroomId AND r.user.role = 'TEACHER'")
